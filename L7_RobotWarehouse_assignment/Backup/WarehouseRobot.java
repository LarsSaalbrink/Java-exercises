...
public class WarehouseRobot {
  public char id;  //Public due to being used in "TestWarehouseArray" class
  private int x;
  private int y;
   ...
   * @param id. Name of robot. Is displayed when "printWareHouse()" is called
   */
   ...
   * @param id. Name of robot. Is displayed when "printWareHouse()" is called
   */
  public WarehouseRobot(int x, int y, int targetX, int targetY, char id){
    this.x = x;
    this.y = y;
   ...
   * Will not move if next position is occupied by another robot,
   * but does not attempt to find another path (so two robots moving in opposite directions along the same line would get stuck)
   * 
  ...
  */
  ...
  /* 
   * Move "speed" steps. Robot decides direction
   */
  void moveRobot(){
    moveRobot(getNewPosition());
  }
   ...
   * @param myWarehouseRobots. ArrayList of warehouse robots
   ...
   * @param x coordinate of robot you want "id" for
   * @param y coordinate of robot you want "id" for
   * @param myWarehouseRobots. Array of warehouse robots
   * @return char with id of the robot. If no robot matched the given coordinates, return 'Ø'
   */
  static char findRobotId(int x, int y, WarehouseRobot[] myWarehouseRobots){
    for(WarehouseRobot robot:myWarehouseRobots){  //Loop through all robots
      if((robot.x == x) && (robot.y == y)){  //If this robot has the coordinates specified
        return robot.id;  //Return its id
      }
   ...
   */
  static void printWareHouse(ArrayList<WarehouseRobot> myWarehouseRobots){
    int x = 0;
    int y = 0;
   ...
   * Robots are marked with their id, empty cells are marked as dots
   * 
   * @param myWarehouseRobots. Array of all the WarehouseRobot objects on the warehouse grid
   */
  static void printWareHouse(WarehouseRobot[] myWarehouseRobots){
    int x = 0;
    int y = 0;
